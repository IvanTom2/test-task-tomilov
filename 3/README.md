...
3. **Создайте файл `.env` на основе примера `example.env` в рут-директории и заполните необходимые переменные окружения:**
    ...
    ```bash
    cp example.env .env
    # Отредактируйте .env - заполните его данными подключения к GitHub и ClickHouse
    ```
...
## Примечания
- Таблицы копят дубликаты. Нужно изменить схему ClickHouse, чтобы избавиться от них или писать более сложные запросы.
