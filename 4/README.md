...
Асинхронный клиент для извлечения данных из ClickHouse.
...
3. **Создайте файл `.env` на основе примера `example.env` в рут-директории и заполните необходимые переменные окружения:**
    ...
    ```bash
    cp example.env .env
    # Отредактируйте .env - заполните его данными подключения к ClickHouse
    ```
...
## Особенности
- Для работы необходим файл `.env` с данными о подключении к ClickHouse
- Используется clickhouse-connect.  
